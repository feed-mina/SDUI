...
| 2026-03-10 | TextField.tsx 데이터 바인딩 우선순위 버그 수정 | `meta.labelText`가 `data[ref_data_id]`보다 먼저 평가되던 버그 수정 → CONTENT_LIST 리피터 실제 데이터 표시 |
| 2026-03-10 | main → lab/claude-dev 머지 충돌 해결 | 13개 파일 충돌 해결: console.log 주석처리(main 기준), diary→content CSS class명, CONTENT_LIST_PAGE→CONTENT_LIST URL, V3 PERFORM setval |
